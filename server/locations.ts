// The district table, owned by the server so an API request can resolve a
// district name to coordinates itself instead of trusting a latitude and
// longitude supplied by the caller. `src/lib/locations.ts` re-exports it.
//
// This module must have no imports: the production image contains only
// `server/` and `dist/`, and it is bundled into the browser build.

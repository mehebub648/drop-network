...
import { DAY_MS, dhakaDate, requestDeadline, resolveRequestTiming } from '../../../server/requestLifecycle';
...
export default function RequestGate({ draft, onDraftChange, user, onClose, onEditSearch, onReady, requireAccount = false }: {
  requireAccount?: boolean; draft: SearchDraft; onDraftChange: (draft: SearchDraft) => void; user: any; onClose: () => void; onEditSearch: () => void; onReady: (id: string) => Promise<void>;
}) {
  const [stage, setStage] = useState<Stage>(requireAccount ? 'account' : draft.requester_role ? 'identity' : 'role');
  ...
  useEffect(() => {
    if (draft.patient_sex === 'MALE' && draft.request_reason === 'CHILDBIRTH') {
      onDraftChange({ ...draft, request_reason: '', request_reason_details: '' });
    }
  }, [draft, onDraftChange]);
  ...
  const submit = (event: FormEvent) => {
    ...
    if (stage === 'date' && !resolveRequestTiming(draft.needed_when || 'SPECIFIC_DATE', draft.needed_date)) return setError('Choose today or a date up to 15 days ahead.');
    ...
    void (async () => {
      ...
      await onReady(id);
    })().catch(cause => { if (cause?.status === 428) setStage('account'); setError(cause.message || 'Could not publish. Your answers are saved.'); }).finally(() => setBusy(false));
  };
  return <ModalPortal onClose={onClose}><div className="dialog-backdrop" role="dialog" aria-modal="true" aria-label="Blood request">
    <section className="action-dialog request-gate-dialog !max-w-2xl !bg-white">
      ...
      {stage === 'account' ? <>
        <div className="mb-6 border-l-4 border-amber-400 bg-amber-50 p-4"><strong>{requestId ? 'Your request is published' : 'Continue with your account'}</strong><p className="mt-2">You can publish three requests and copy three donor numbers per request without an account. Continue here to unlock more.</p>{requestId && <Link className="mt-2 inline-block text-primary underline" to={`/request/${requestId}`}>Manage this request</Link>}<p className="mt-2 text-sm">Until you sign in, keep this browser’s data to manage the post. Device access ends at its deadline.</p></div>
        {error && <p role="alert" className="dialog-error">{error}</p>}
        {user?.is_verified ? <button type="button" className="button button-primary" onClick={() => void onReady(requestId).catch(cause => setError(cause.message))}>Continue to donor contact</button> : <AccountFlow initialPhone={user?.phone} verifyOnly={Boolean(user)} onComplete={async () => { if (requestId) await onReady(requestId); else setStage('review'); }} />}
        ...
        {stage === 'date' && <><label>When is blood needed?<Select required value={draft.needed_when || 'SPECIFIC_DATE'} onChange={event => { const mode = event.target.value as NonNullable<SearchDraft['needed_when']>; const timing = resolveRequestTiming(mode, draft.needed_date); set({ needed_when: mode, ...(timing ? { needed_date: timing.needed_date } : {}) }); }}><option value="ASAP">As soon as possible (urgent)</option><option value="TODAY">Later today</option><option value="THIS_WEEK">Within this week (next 7 days)</option><option value="SPECIFIC_DATE">At a specific date</option></Select></label>{(!draft.needed_when || draft.needed_when === 'SPECIFIC_DATE') && <label>Needed date<DateInput className="input" required min={dhakaDate()} max={dhakaDate(Date.now() + 15 * DAY_MS)} value={draft.needed_date || ''} onChange={event => set({ needed_date: event.target.value })} /></label>}</>}

...
export default function ContactReportsPage({ user, onUpdate }: ProfilePageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {(inbox?.issues.WRONG_NUMBER || inbox?.issues.UNREACHABLE || inbox?.suspended) && <Surface className="p-6"><h3 className="text-lg font-extrabold text-slate-950">Reverify {user.phone}</h3><p className="mt-2 text-sm text-slate-600">A fresh Messavo code clears earlier wrong-number and connection warnings and restores an automatically suspended profile.</p>{phoneStep === 'idle' ? <button type="button" disabled={busy} onClick={() => void requestPhoneCode()} className="primary-button mt-5"><Phone className="h-4 w-4" />Send verification code</button> : <form onSubmit={verifyPhone} className="mt-5 space-y-4"><OtpDeliveryStatus delivery={delivery} onDeliveryChange={setDelivery} busy={busy} onResend={() => void requestPhoneCode()} /><label className="block"><span className="mb-2 block text-sm font-bold">Six-digit code</span><input required autoFocus inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={code} onChange={event => setCode(event.target.value.replace(/\D/g, ''))} className="input" /></label><button disabled={busy || code.length !== 6} className="primary-button"><KeyRound className="h-4 w-4" />Verify and clear</button></form>}</Surface>}

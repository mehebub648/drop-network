...
export default function PrivacyPage() {
  return (
    <InfoPage eyebrow="Legal" title="Privacy Policy" intro="Effective 7 September 2026. This policy explains what Drop currently collects, why it is used, where it is stored, and when another member can see it.">
      <section>
        <h2>Information we collect</h2>
        <p>When you create or use an account, Drop can store your name, private account phone number, private date of birth and home location, password hash, optional donor blood group, district and approximate district coordinates, availability, an optional private reason when you are unavailable, optional self-reported age and weight, an optional private description of a medical condition or current sickness, verification status, your exact or approximate last-donation declaration, lifetime donation count, and private donation-history entries. Blood requests can include patient and requester names, blood group, blood component, units needed, a broad reason for the blood need, an optional short broad description when “Other” is selected, district, needed-by date, contact people, comments, status, and creation time. Community posts store the title, Markdown body, post type, author account, publication state and times, and one processed image for a donation story when you choose to upload it.</p>
      </section>
      <section>
        <h2>How we use it</h2>
      ...
      <section>
        ...
        <p>Public donor searches can show an opted-in member’s name, blood group, approximate location or distance, verification signal, current availability, and the self-reported last-donation summary and lifetime donation count when provided, but never their detailed donation records or hospital and organization names. A matching request owner can explicitly copy a donor number. Guests can copy three distinct donor numbers per request; further access requires a verified account. Missing availability is treated as available for ordering, while clinical deferrals remain respected.</p>
        ...
        <p className="mt-3">Each reveal is recorded and limited to the request's results. The selected upazila appears first; below 30 matches, other upazilas in the same district may appear below it. Save a call outcome before opening another number. A number is for this request only and must not be reshared or collected for another purpose.</p>
        ...
        <p className="mt-3">Public request lists do not include phone numbers, but an active request's detail page publishes the patient's name, the selected broad reason and optional broad “Other” description, and the chosen patient-side contact so potential donors can understand the request and call immediately. The patient name and number stop being public when the request is fulfilled, cancelled, or expires. Only submit details you have permission to publish, and do not include a diagnosis or medical report in the reason description. Information you add to comments may be visible publicly, so do not post unrelated phone numbers or sensitive medical details there.</p>
      ...
      <section>
        ...
        <p>Guests may publish an explicitly consented patient-side contact without signing in. That number is not independently verified and is never filled from the private account phone. Guest posts expire at the end of the selected Bangladesh date; account-owned posts remain live for 30 more days unless closed. Signing in on the same device adopts its unexpired guest posts. Losing browser storage loses guest management access. Guests may publish three requests total and copy three distinct numbers per request. Continuing beyond either limit opens guided account questions in the same workflow; optional donor enrollment permits protected sharing of the account phone as a donor contact, never as requester contact.</p>
        <p>Phone-first sign-in indicates whether a password account exists. Phone, device, and IP limits reduce abuse, but this is not an account-enumeration-proof flow.</p>
        <p>Drop currently stores application records in a LanceDB datastore. Passwords and verification codes are bcrypt-hashed. Login state uses an httpOnly, SameSite session cookie that browser JavaScript cannot read. Guest posts use a separate server-issued secret device credential in an HttpOnly, Secure cookie, with only its hash stored on the server. Request management grants expire at each request deadline. After 15 days without active use, an unclaimed guest identity, its remaining request/contact activity, and comments are deleted from active storage. Signing in links its unexpired activity to the account. Local patient drafts expire after 15 days. Restricted historical backups may retain older records until those backups are retired. Legacy fingerprints never authorize request ownership. Security headers, JSON-only request handling, restricted production CORS, and rate limits reduce common risks, but no online service can promise absolute security.</p>
      </section>
      <section>

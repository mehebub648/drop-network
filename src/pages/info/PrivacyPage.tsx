...
export default function PrivacyPage() {
  return (
    <InfoPage eyebrow="Legal" title="Privacy Policy" intro="Effective 4 September 2026. This policy explains what Drop currently collects, why it is used, where it is stored, and when another member can see it.">
      <section>
      ...
      <section>
        ...
        <p>Drop currently stores application records in a LanceDB datastore. Passwords and verification codes are bcrypt-hashed. Login state uses an httpOnly, SameSite session cookie that browser JavaScript cannot read. Guest posts use a separate server-issued secret device credential in an HttpOnly, Secure cookie, with only its hash stored on the server. Request management grants expire at each request deadline. A legacy random <code>drop_fingerprint</code> remains only for comment attribution, not request ownership. Security headers, JSON-only request handling, restricted production CORS, and rate limits reduce common risks, but no online service can promise absolute security.</p>
      </section>
      <section>
